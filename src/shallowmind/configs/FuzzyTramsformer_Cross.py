...
model_para=dict(
    ...
    model_ckpt=None,
    fixed=False,
    attention_type='only_cross',
)

exp_name=f"CFTF-ds={dataset_name}_{dataset_para['type'][-1]}-d={model_para['depth']}-nh={model_para['num_heads']}-r={model_para['num_rules']}-bs={batch_size}-ls={base_lr}-label={dataset_para['label']}"
