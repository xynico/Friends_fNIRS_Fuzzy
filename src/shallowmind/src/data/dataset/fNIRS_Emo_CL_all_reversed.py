...
class fNIRS_Emo_CL_all_reversed(torch.utils.data.Dataset):
    def __init__(self,
                data_root = None,
                test_size=0.3,
                reversed = 0,
    ...
    def gen_data_index_table(self):
        ...
        if os.path.exists(os.path.join(self.temp_save_folder, f'data_index_table_{self.tier}_{self.dataset_random_state}.csv')):
            self.data_index_table = pd.read_csv(os.path.join(self.temp_save_folder, f'data_index_table_{self.tier}_{self.dataset_random_state}.csv'))
        else:
            ...
            self.data_index_table.to_csv(os.path.join(self.temp_save_folder, f'data_index_table_{self.tier}_{self.dataset_random_state}.csv'), index=False)
    ...
    def __getitem__(self, idx):
        ...
        data = einops.rearrange(data, 'k t f -> k f t')
