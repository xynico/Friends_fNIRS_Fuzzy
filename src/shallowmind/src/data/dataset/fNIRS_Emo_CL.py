...
class fNIRS_Emo_CL(torch.utils.data.Dataset):
    def __init__(self,
                data_root = None,
                test_size=0.3,
    ...
    def __getitem__(self, idx, emotion_label=None, pair_idx=None, isubj=None, trial_idxes=None, hand=None, relationship_label=None):
        ...
        if self.CL_way == 'subj':
            ...
            if isubj is None:
                isubj = np.random.choice([0,1], size=1, replace=False)[0]
            
            if trial_idxes is None:
                trial_idxes = np.random.choice(self.data_table[(self.data_table['pair_id']==pair_idx)  & (self.data_table['isubj']==isubj) & (self.data_table['emotion_label']==emotion_label)]['trial_idx'].values[0], size=2, replace=False)
            trial_idx1, trial_idx2 = trial_idxes[0], trial_idxes[1]
            
            # get the data
            data1 = torch.tensor(self.epoch_data[int(hand)][int(relationship_label)][emotion_label][pair_idx][isubj][trial_idx1], dtype=torch.float32)
            data2 = torch.tensor(self.epoch_data[int(hand)][int(relationship_label)][emotion_label][pair_idx][isubj][trial_idx2], dtype=torch.float32)
            ...
            subj_id = f"{emotion_label}{relationship_label}{hand}{key}{isubj}"
        
        elif self.CL_way == 'pair':

            # if self.tier in ['val', 'test']:
            #     pair_idx = np.random.choice(self.data_table['pair_id'].unique(), size=1, replace=False)[0] # pair_idx = f"{hand}-{relationship_label}-{key[7:]}"
            #     hand, relationship_label, key = pair_idx.split('-')
            # elif self.tier == 'train':
            if relationship_label is None:
                relationship_label = np.random.choice([0,1], size=1, replace=False)[0]
            if hand is None:
                hand = np.random.choice([2,1], size=1, replace=False)[0]
            if emotion_label is None:
                emotion_label = np.random.choice([2,3], size=1, replace=False)[0]
            if pair_idx is None:
                all_pair_idx = self.data_table[(self.data_table['relationship_label']==relationship_label) & (self.data_table['hand']==hand)]['pair_id'].values
                pair_idx = np.random.choice(all_pair_idx, size=1, replace=False)[0]
            
            key = pair_idx.split('-')[2]
            subj_id = f"{emotion_label}{relationship_label}{hand}{key}"
            if trial_idxes is None:
