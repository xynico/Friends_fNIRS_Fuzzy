...
class Fuzzy_Attention(nn.Module):
    ...
    def forward(self,x):
        ...
        conq=self.to_c(v_rearranged) # (b s t) r
        conq_rearranged=rearrange(conq, '(b s t) r-> (b s) r t',b=b, s=s, t=t, r=self.n_rules)
        output=Fss @ conq_rearranged # (b s) c t
        output=rearrange(output, '(b s) c t-> b s c t',b=b, s=s, c=c, t=t)
        return output
