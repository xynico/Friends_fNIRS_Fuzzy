...
class WarmupCosineDecayLR(torch.optim.lr_scheduler._LRScheduler):
    ...
    def __init__(self, optimizer, max_epochs=-1, max_steps=-1, warmup_epochs=40, min_lr=0, last_epoch=-1):
    ...
    def get_lr(self):
        ...
        if self.last_epoch < self.warmup_epochs:
            ...
            # Post max_epochs, use min_lr
            return [self.min_lr for _ in self.base_lrs]
        else:
            # Cosine decay phase
            cos_decay = 0.5 * (1 + math.cos(math.pi * (self.last_epoch - self.warmup_epochs) / (self.max_epochs - self.warmup_epochs)))
            return [base_lr * cos_decay + self.min_lr for base_lr in self.base_lrs]
