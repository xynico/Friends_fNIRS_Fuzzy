...
class ModelInterface(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        meta_data = output['meta_data'] if 'meta_data' in output else None


        # logging all output
        for name, value in output.items():
            if name != 'output' and name != 'loss' and name != 'meta_data' and name != 'label':
        ...
        # return {'meta_data': meta_data, 'pred': output['output'], 'label': input['seq']}
        return {'meta_data': meta_data, 'pred': output['output'], 'label': output['label']}

    def validation_epoch_end(self, validation_step_outputs):
        meta_data = [out['meta_data'] for out in validation_step_outputs]   
    ...
    def test_epoch_end(self, test_step_outputs):
        ...
        # [test_step_output1, test_step_output2, ...]
        meta_data = [out['meta_data'] for out in test_step_outputs]     
